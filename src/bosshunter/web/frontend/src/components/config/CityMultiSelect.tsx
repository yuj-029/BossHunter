  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-2">
        <input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索城市名或 BOSS 编码" className="min-w-0 flex-1 rounded-xl border border-card-border bg-surface-subtle px-3 py-2 text-xs outline-none focus:border-primary" />
        <Button type="button" variant="secondary" size="sm" onClick={onRefresh} disabled={refreshing}>{refreshing ? '刷新中...' : '刷新城市列表'}</Button>
      </div>
      <div className="flex max-h-44 flex-wrap gap-2 overflow-y-auto rounded-xl border border-card-border bg-surface-subtle p-2">
        {visible.map(city => {
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-muted">
        ...
        {value.map(name => (
          <button key={name} type="button" onClick={() => toggle(name)} className="rounded-full bg-surface-accent px-2 py-1 text-primary" title="点击移除">
            {name} ×
          </button>
        ))}
      </div>
